...
import { ArrowLeft, Grid, Box, Terminal, FileText, AlertCircle, Edit } from 'lucide-react';
...
const DaemonSetDetail = () => {
    ...
    const [daemonset, setDaemonSet] = useState(null);
    ...
    const fetchData = async () => {
        try {
            const data = await fetch(`/api/v1/daemonsets/${namespace}/${name}`).then(res => res.json());
            setDaemonSet(data);
        } catch (error) {
    ...
    if (!daemonset) {
        return (
            <div className="text-center py-12">
                ...
                <h2 className="text-2xl font-bold mb-2">DaemonSet Not Found</h2>
                <button
                    onClick={() => navigate('/daemonsets')}
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <div className="flex-1">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                            ...
                            <h1 className="text-3xl font-bold tracking-tight">{daemonset.name}</h1>
                            <Badge variant="default">{daemonset.namespace}</Badge>
                        </div>
                        <button
                    ...
                    <div className="flex items-center space-x-4 mt-1 text-sm text-muted-foreground">
                        <span>Desired: {daemonset.desired}</span>
                        <span>Current: {daemonset.current}</span>
                        <span>Ready: {daemonset.ready}</span>
                        <span>Age: {daemonset.age}</span>
                    </div>
                </div>
            </div>

            {/* Images */}
            {daemonset.images && daemonset.images.length > 0 && (
            ...
            {/* Pods */}
            <div className="rounded-xl glass-card p-6">
                <h3 className="text-lg font-semibold mb-4">Pods ({daemonset.pods?.length || 0})</h3>
                <Table columns={podColumns} data={daemonset.pods || []} />
            </div>
